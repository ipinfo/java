...
public class IPRequestPlus extends BaseRequest<IPResponsePlus> {
    private final static String URL_FORMAT = "https://api.ipinfo.io/lookup/%s";
    private final String ip;

    public IPRequestPlus(OkHttpClient client, String token, String ip) {
        super(client, token);
        this.ip = ip;
    }

    @Override
    public IPResponsePlus handle() throws RateLimitedException {
        if (IPRequest.isBogon(ip)) {
            try {
                return new IPResponsePlus(ip, true);
            } catch (Exception ex) {
        ...
        try (Response response = handleRequest(request)) {
            ...
            try {
                return gson.fromJson(response.body().string(), IPResponsePlus.class);
            } catch (Exception ex) {
                throw new ErrorResponseException(ex);
            }
        }
